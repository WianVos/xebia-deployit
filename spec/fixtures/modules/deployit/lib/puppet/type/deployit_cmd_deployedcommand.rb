Puppet::Type.newtype(:deployit_cmd_deployedcommand ) do

      # this type is generated with genres.rb
      # generated for deployit 3.8.5

      desc 'adds a deployit_cmd_deployedcommand ci to a remote deployit server'

      ensurable do

        desc 'deployit deployit_cmd_deployedcommand resource state'
      ...
      newparam(:id, :namevar => true) do
        desc 'deployit_cmd_deployedcommand id (full path required)'
      ...
      newproperty(:commandline) do
          ...
          validate do |value|
            unless value != 'unset'
              fail('commandline needs to be set')
            end
          end
