Puppet::Type.newtype(:deployit_wls_deployedfolder ) do

      # this type is generated with genres.rb
      # generated for deployit 3.8.5

      desc 'adds a deployit_wls_deployedfolder ci to a remote deployit server'

      ensurable do

        desc 'deployit deployit_wls_deployedfolder resource state'
      ...
      newparam(:id, :namevar => true) do
        desc 'deployit_wls_deployedfolder id (full path required)'
      ...
      newproperty(:targetdirectory) do
          ...
          defaultto ('${deployed.container.domain.domainHome}/lib') 
        
           
         
          validate do |value|
            unless value != 'unset'
              fail('targetdirectory needs to be set')
            end
          end
