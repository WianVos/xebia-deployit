Puppet::Type.newtype(:deployit_wls_uniformdistributedtopic ) do

      # this type is generated with genres.rb
      # generated for deployit 3.8.5

      desc 'adds a deployit_wls_uniformdistributedtopic ci to a remote deployit server'

      ensurable do

        desc 'deployit deployit_wls_uniformdistributedtopic resource state'
      ...
      newparam(:id, :namevar => true) do
        desc 'deployit_wls_uniformdistributedtopic id (full path required)'
      ...
      [ "deployit_wls_jmstarget",  "deployit_core_directory", ].each {|c|
